...
export async function POST(request: Request) {
        ...
        const waitForAnyTableInFrames = async (timeoutMs = TIMEOUT_EL) => {
            ...
            while (Date.now() - start < timeoutMs) {
                ...
                for (const frame of frames) {
                    try {
                        const count = await frame.$$eval('table', (tables: any[]) => tables.length);
                        if (count > 0) return;
                    } catch {
        ...
        const pushDebug = async (key: string) => {
            ...
            try {
                debug[key] = {
                    ...
                    frames: page.frames().map((f: any) => f.url()).slice(0, 10),
    ...
    try {
        ...
        const latestCookies: SiakadSession['cookies'] = latestCookiesRaw.map((c: any) => ({
            name: c.name,
            value: c.value,
            domain: c.domain,
            path: c.path,
